...
const mockClaimStatus = [
  ...
  },
  {
    ...
    RejectionReason:
      "Submitted documents were not sufficient to establish the claim as per Section 4(3) of the Act. Applicant may re-apply with required evidence.",
  },
];
...
const ClaimStatusDetails = ({ claim }) => {
  ...
  const processSteps = [
    {
      name: "Gram Sabha and Forest Rights Commitee",
      substeps: [
        "Claims Collected",
        "Evidence verified",
        "Gram Sabha resolution passed",
        "Forwarded to SDLC",
      ],
    },
    {
      name: "Sub-Division level commitee",
      substeps: [
        "Resolution review",
        "dispute resolution",
        "Draft Record prepared",
        "Forwarded to DLC",
      ],
    },
    {
      name: "District level commitee",
      substeps: [
        "appeal filed, if any",
        "Title Issued",
        "Case forwarded for monitoring to SLC",
      ],
    },
  ...
  const isSubStepApproved = (substepName) => {
    const info = claim.Information.find(
      (info) => info.Sub_Step === substepName
    );
  ...
  switch (claim.Status) {
        ...
        <div className="status-details-content">
          {processSteps.map((step, index) => {
            ...
            return (
              <div
                key={index}
                className="step-heading completed"
                style={{
                  display: "flex",
                  alignItems: "center",
    ...
    case 0: // Rejected
      ...
      const failedStep = processSteps.find((step) =>
        step.substeps.includes(lastInfo.Sub_Step)
      );
      return (
        <div className="status-details-content">
          <div className="step-heading rejected">❌ {failedStep.name}</div>
          <div className="substep-item rejected">
            Rejected at: {lastInfo.Sub_Step}
          </div>
      ...
      return (
        <div className="status-details-content">
          {processSteps.map((step, index) => {
            ...
            if (stepNumber < claim.Status) {
              // Already completed steps
              return (
                <div key={index} className="step-heading completed">
                  ✅ {step.name}
                </div>
              );
            }
            if (stepNumber > claim.Status) {
              // Future steps
              return (
                <div key={index} className="step-heading future">
                  {step.name}
                </div>
              );
            }
                  ...
                  {step.substeps.map((substep, subIndex) => (
                    <li
                      ...
                      className={`substep-item ${
                        isSubStepApproved(substep) ? "completed" : "pending"
                      }`}
                    >
  ...
  return (
    <div className="claim-item">
      <div className="status-query-section">
        ...
        <div
          className={`placeholder-box status-${getStatusText(
            claim.Status
          ).toLowerCase()}`}
        >
...
export default function MyYojnaBook() {
  ...
  const handleRecommendationClick = () => {
    ...
    // Start timeout AFTER tab is active
    setTimeout(() => {
      setRecommendLoading(false);
      ...
      <div className="dashboard-page">
        ...
        <header className="top-menu">
          ...
          <nav className="navbar">
            ...
            <a href="/CitizenDashboard" className="nav-item">
              Dashboard
            </a>
            <a href="#" className="nav-item">
              About
        ...
        <main className="main-content">
          <aside className="sidebar">
            ...
            <button
              className={`sidebar-button ${
                activeTab === "Meri Yojna" ? "active" : ""
              }`}
              onClick={() => setActiveTab("Meri Yojna")}
            >
              Meri Yojna
            </button>
                  ...
                  const recommendData = [
                    {
                      ...
                      About:
                        "Pradhan Mantri Kisan Samman Nidhi is an initiative by the government of India that gives farmers up to ₹6,000 per year as minimum income support. The initiative was announced by Piyush Goyal during the 2019 Interim Union Budget of India on 1 February 2019.",
                      Contact: "9812930981",
                    },
                    {
                      Scheme_Name: "Eklavya Model Residential Schools (EMRS)",
                      About:
                        "A scheme for providing quality middle and high level education to Scheduled Tribe students in remote areas, not only to enable them to avail of reservation in high and professional educational courses and get jobs in government and public and private sectors, but also to have access to the best opportunities in education at par with the non ST population.",
                      Contact: "email@example.gov.in",
                    },
                    {
                      Scheme_Name: "Pradhan Mantri Van Dhan Yojana",
                      About:
                        "An initiative targeting livelihood generation for tribal gatherers and transforming them into entrepreneurs. The idea is to set-up tribal community owned Minor Forest Produce-centric multi-purpose enterprises.",
